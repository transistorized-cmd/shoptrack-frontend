...
export function vueI18nProductionFix() {
  return {
    ...
    generateBundle(options, bundle) {
      const isProd =
        process.env.NODE_ENV === "production" || options.format === "es";
      ...
      if (isProd) {
        ...
        Object.keys(bundle).forEach((fileName) => {
          ...
          if (chunk.type === "chunk" && chunk.code) {
            ...
            if (
              chunk.code.includes("eval(") ||
              chunk.code.includes("Function(")
            ) {
              console.warn(
                `⚠️  Found eval/Function in ${fileName} - this may cause CSP violations`,
              );
            }

            // Check for message compiler signatures
            if (
              chunk.code.includes("compileToFunction") ||
              chunk.code.includes("compile(")
            ) {
