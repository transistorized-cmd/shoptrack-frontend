...
describe("LocalizedDateInput", () => {
  ...
  describe("Calendar Display", () => {
    ...
    it("highlights today with special styling", async () => {
      ...
      const todayElement = wrapper.find(
        ".font-bold.text-blue-500, .bg-blue-500",
      );
  ...
  describe("Date Selection", () => {
    ...
    it("navigates to next month when clicking next month day", async () => {
      ...
      const dayButtons = wrapper.findAll(".grid.grid-cols-7 button");
      const nextMonthDay = dayButtons
        .filter((btn: any) => btn.classes().includes("text-gray-400"))
        .at(-1);
  ...
  describe("Calendar Grid Generation", () => {
    it("generates correct number of days for the month", async () => {
      ...
      const currentMonthDays = dayButtons.filter(
        (btn: any) => !btn.classes().includes("text-gray-400"),
      );
  ...
  describe("Styling and Classes", () => {
    it("applies correct classes to current month days", async () => {
      ...
      const currentMonthDay = wrapper
        .findAll(".grid.grid-cols-7 button")
        .find(
          (btn: any) =>
            btn.text() === "10" && !btn.classes().includes("text-gray-400"),
        );
  ...
  describe("Accessibility", () => {
    ...
    it("provides navigation buttons with clear icons", async () => {
      ...
      const navButtons = wrapper
        .findAll("button")
        .filter((btn: any) => btn.find("svg").exists());
