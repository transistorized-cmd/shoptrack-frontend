...
describe('useAsyncErrorHandler', () => {
  ...
  describe('basic functionality', () => {
    ...
    it('should maintain readonly state properties through operations', () => {
      // Arrange
      const handler = useAsyncErrorHandler({ showNotification: false, logError: false })
  ...
  describe('executeAsync', () => {
    ...
    it('should handle operation errors', async () => {
      ...
      // Act & Assert
      await expect(handler.executeAsync(mockOperation)).rejects.toThrow('Operation failed')
    ...
    it('should fail after exhausting retries', async () => {
      ...
      // Act & Assert
      const resultPromise = expect(handler.executeAsync(mockOperation)).rejects.toThrow('Persistent failure')
    ...
    it('should handle non-Error objects', async () => {
      ...
      // Act & Assert
      await expect(handler.executeAsync(mockOperation)).rejects.toBeInstanceOf(Error)
  ...
  describe('error message formatting', () => {
    it('should format network errors', () => {
      ...
      // Assert
      expect(handler.errorMessage.value).toBe('Network connection error. Please check your internet connection.')
    })
    ...
    it('should format 404 errors', () => {
      ...
      // Assert
      expect(handler.errorMessage.value).toBe('The requested resource was not found.')
    })
    ...
    it('should format 403 errors', () => {
      ...
      // Assert
      expect(handler.errorMessage.value).toBe('You do not have permission to access this resource.')
    })
    ...
    it('should format 500 errors', () => {
      ...
      // Assert
      expect(handler.errorMessage.value).toBe('Server error occurred. Please try again later.')
    })
    ...
    it('should format chunk load errors', () => {
      ...
      // Assert
      expect(handler.errorMessage.value).toBe('Failed to load application resources. Please refresh the page.')
    })
  ...
  describe('retry mechanism', () => {
    ...
    it('should not retry when retryCount is 0', async () => {
      ...
      // Act & Assert
      await expect(handler.executeAsync(mockOperation)).rejects.toThrow('No retry test')
      expect(mockOperation).toHaveBeenCalledTimes(1)
    })
  })
})
...
describe('setupGlobalAsyncErrorHandler', () => {
  ...
  it('should handle unhandled promise rejections', () => {
    // Arrange
    setupGlobalAsyncErrorHandler()
    const unhandledRejectionHandler = mockAddEventListener.mock.calls
  ...
  it('should handle vue async error events', () => {
    // Arrange
    setupGlobalAsyncErrorHandler()
    const asyncErrorHandler = mockAddEventListener.mock.calls
      .find(call => call[0] === 'vue:async-error')?.[1]
...
describe('edge cases and error scenarios', () => {
  ...
  it('should handle errors with special characters', () => {
    // Arrange
    const handler = useAsyncErrorHandler({ showNotification: false, logError: false })
    const specialError = new Error('Error with émojis 🚀 and spéciàl châractérs')

    // Act
    handler.handleError(specialError)

    // Assert
    expect(handler.errorMessage.value).toBe('Error with émojis 🚀 and spéciàl châractérs')
  })
