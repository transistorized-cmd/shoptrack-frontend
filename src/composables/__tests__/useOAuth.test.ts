  ...
  describe("Google OAuth", () => {
    describe("Script Loading", () => {
      ...
      it("should handle Google script loading failure", async () => {
        ...
        expect(oauth.error.value).toContain(
    ...
    describe("OAuth Flow", () => {
      ...
      it("should initialize Google OAuth with correct configuration", async () => {
        mockWindow.google.accounts.id.initialize.mockImplementation(
          ({ callback }) => {
            setTimeout(() => {
              callback({ credential: "test-credential" });
            }, 0);
      ...
      it("should handle auth store login failure", async () => {
        mockAuthStore.loginWithOAuth.mockRejectedValue(
          new Error("Auth store error"),
        );
      ...
      it("should fallback to popup when prompt is not displayed", async () => {
        ...
        mockWindow.google.accounts.oauth2.initTokenClient.mockImplementation(
          (config) => {
        ...
        expect(
          mockWindow.google.accounts.oauth2.initTokenClient,
