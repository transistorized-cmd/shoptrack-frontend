...
describe('usePluginLocalization', () => {
  ...
  describe('getLocalizedPluginName', () => {
    ...
    it('should handle plugin keys with special characters', () => {
      ...
      // Assert
      expect(mockT).toHaveBeenCalledWith('plugins.plugin-with.special_chars.name')
  ...
  describe('getLocalizedPluginDescription', () => {
    ...
    it('should handle long descriptions', () => {
      // Arrange
      const longDescription = 'This is a very long plugin description that contains multiple sentences and provides detailed information about the plugin functionality and capabilities.'
      const plugin: ReceiptPlugin = {
        key: 'long-desc',
      ...
      const translatedDescription = 'Esta es una descripción muy larga del plugin que contiene múltiples oraciones y proporciona información detallada sobre la funcionalidad y capacidades del plugin.'
  ...
  describe('localizePlugins', () => {
    ...
    it('should handle large plugin arrays efficiently', () => {
      // Arrange
      const plugins: ReceiptPlugin[] = Array.from({ length: 100 }, (_, index) => ({
        key: `plugin-${index}`,
        name: `Plugin ${index}`,
  ...
  describe('integration tests', () => {
    it('should work with different localization scenarios', () => {
      ...
      expect(getLocalizedPluginDescription(receiptPlugin)).toBe('Procesar archivos PDF')
      expect(getLocalizedPluginName(reportPlugin)).toBe('Reporte de Análisis')
      expect(getLocalizedPluginDescription(reportPlugin)).toBe('Generar análisis')
    ...
    it('should handle i18n function errors gracefully', () => {
      ...
      // Act & Assert
      const { getLocalizedPluginName, getLocalizedPluginDescription } = usePluginLocalization()

      expect(() => getLocalizedPluginName(plugin)).toThrow('Translation error')
      expect(() => getLocalizedPluginDescription(plugin)).toThrow('Translation error')
    })
    ...
    it('should maintain consistent behavior across all functions', () => {
      ...
      plugins.forEach((plugin, index) => {
        ...
        expect(individualDescription).toBe(`${index === 0 ? 'Primera' : 'Segunda'} prueba consistente`)
