...
vi.mock('@/views/CategoryAnalytics.vue', () => ({
  default: { name: 'CategoryAnalytics', template: '<div>CategoryAnalytics</div>' }
}));
...
describe('Router', () => {
  ...
  describe('Route Configuration', () => {
    ...
    it('should have error handling routes', () => {
      ...
      const notFoundRoute = routes.find(r => r.path === '/404' && r.name === 'not-found');
    ...
    it('should use lazy loading for most components', () => {
      ...
      const lazyLoadedRoutes = [
        ...
        'receipts', 'upload', 'receipt-detail', 'reports',
        'category-analytics', 'price-trends', 'profile', 'not-found'
      ];
  ...
  describe('Authentication Guards', () => {
    describe('Protected Routes', () => {
      ...
      it('should preserve query parameters when redirecting to login', async () => {
        ...
        expect(testRouter.currentRoute.value.name).toBe('login');
        expect(testRouter.currentRoute.value.query?.redirect).toBe('/receipts?status=pending&page=2');
      });
      ...
      it('should handle protected analytics routes', async () => {
        ...
        expect(testRouter.currentRoute.value.name).toBe('login');
        expect(testRouter.currentRoute.value.query?.redirect).toBe('/analytics/categories');
      });
    });
    ...
    describe('Auth Store Initialization', () => {
      ...
      it('should handle auth store initialization errors gracefully', async () => {
        mockAuthStore.isAuthenticated = false;
        mockAuthStore.loading = false;
        mockAuthStore.initialize = vi.fn().mockRejectedValue(new Error('Init failed'));
    ...
    describe('Route Parameters and Queries', () => {
      ...
      it('should preserve complex query strings in redirects', async () => {
        ...
        await testRouter.push('/reports?dateFrom=2024-01-01&dateTo=2024-12-31&category=food');

        expect(testRouter.currentRoute.value.query?.redirect).toBe('/reports?dateFrom=2024-01-01&dateTo=2024-12-31&category=food');
      });
      ...
      it('should handle hash fragments in redirects', async () => {
        ...
        expect(testRouter.currentRoute.value.query?.redirect).toBe('/profile#settings');
      });
    });
  ...
  describe('Route Meta Properties', () => {
    it('should correctly identify routes requiring authentication', () => {
      ...
      const authRequiredRoutes = routes.filter(route =>
      ...
      const expectedAuthRoutes = [
        'home', 'receipts', 'upload', 'receipt-detail',
        'reports', 'category-analytics', 'price-trends', 'profile'
      ];

      expectedAuthRoutes.forEach(routeName => {
        const found = authRequiredRoutes.some(route => route.name === routeName);
    ...
    it('should correctly identify guest-only routes', () => {
      ...
      const guestOnlyRoutes = routes.filter(route =>
    ...
    it('should have routes without special meta requirements', () => {
      ...
      const publicRoutes = routes.filter(route =>
        !route.meta?.requiresAuth && !route.meta?.requiresGuest
      );
