...
describe("pluginsService", () => {
  ...
  describe("getAllPlugins", () => {
    ...
    it("should handle API errors and log them", async () => {
      ...
      // Act & Assert
      await expect(pluginsService.getAllPlugins()).rejects.toThrow(
        "Network error",
      );
  ...
  describe("getPluginStatistics", () => {
    it("should fetch plugin statistics successfully", async () => {
      ...
      // Assert
      expect(mockApiWithTimeout.fast.get).toHaveBeenCalledTimes(1);
      expect(mockApiWithTimeout.fast.get).toHaveBeenCalledWith(
        "/plugins/statistics",
      );
    ...
    it("should handle statistics API errors", async () => {
      ...
      // Act & Assert
      await expect(pluginsService.getPluginStatistics()).rejects.toThrow(
  ...
  describe("detectPlugin", () => {
    ...
    it("should detect compatible plugin for PDF file", async () => {
      ...
      expect(result.message).toBe(
        "Detected Amazon Receipt Parser for .pdf file",
      );
    });

    it("should detect generic plugin when only generic supports file type", async () => {
      // Arrange
      const mockFile = new File(["test"], "receipt.txt", {
        type: "text/plain",
      });
    ...
    it("should prefer specific plugins over generic ones", async () => {
      ...
      const shuffledPlugins = [
        mockAvailablePlugins[2],
        mockAvailablePlugins[0],
        mockAvailablePlugins[1],
      ];
    ...
    it("should handle files without extensions", async () => {
      // Arrange
      const mockFile = new File(["test"], "receipt", {
    ...
    it("should handle files with multiple dots in name", async () => {
      // Arrange
      const mockFile = new File(["test"], "receipt.backup.2024.pdf", {
        type: "application/pdf",
      });
  ...
  describe("uploadWithAutoDetection", () => {
    it("should upload file with auto detection successfully", async () => {
      // Arrange
      const mockFile = new File(["test content"], "receipt.pdf", {
        type: "application/pdf",
      });
      const mockResponse = {
        success: true,
    ...
    it("should handle upload progress callback", async () => {
      ...
      // Act
      await pluginsService.uploadWithAutoDetection(
        mockFile,
    ...
    it("should handle upload errors and log them", async () => {
      ...
      // Act & Assert
      await expect(
        pluginsService.uploadWithAutoDetection(mockFile),
      ).rejects.toThrow();
      expect(mockErrorLogger.logApiError).toHaveBeenCalledWith(
        expect.any(Error),
        "/upload",
    ...
    it("should create proper FormData", async () => {
      ...
      // Assert
      const formDataArg = mockApiWithTimeout.claudeUpload.post.mock
  ...
  describe("uploadWithSpecificPlugin", () => {
    it("should upload file with Claude plugin using Claude upload timeout", async () => {
      ...
      // Act
      const result = await pluginsService.uploadWithSpecificPlugin(
        mockFile,
        pluginKey,
      );
    ...
    it("should handle upload progress with specific plugin", async () => {
      ...
      // Act
      await pluginsService.uploadWithSpecificPlugin(
        mockFile,
        pluginKey,
        mockProgressCallback,
      );
  ...
  describe("service integration", () => {
    it("should work with getAllPlugins and detectPlugin in sequence", async () => {
      ...
      const detection = await pluginsService.detectPlugin(
        mockFile,
        plugins.receiptPlugins,
      );
