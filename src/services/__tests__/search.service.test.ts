...
describe('SearchService', () => {
  ...
  describe('search (GET method)', () => {
    ...
    it('should handle API errors and log them', async () => {
      ...
      // Act & Assert
      await expect(service.search(searchRequest)).rejects.toThrow('Failed to perform search')
      expect(mockConsole.error).toHaveBeenCalledWith('Search failed:', mockError)
      expect(mockApiWithTimeout.fast.get).toHaveBeenCalledTimes(1)
    })
    ...
    it('should handle network timeout errors', async () => {
      ...
      // Act & Assert
      await expect(service.search(searchRequest)).rejects.toThrow('Failed to perform search')
      expect(mockConsole.error).toHaveBeenCalledWith('Search failed:', timeoutError)
    })
    ...
    it('should handle server errors with different status codes', async () => {
      ...
      for (const error of serverErrors) {
        ...
        // Act & Assert
        await expect(service.search(searchRequest)).rejects.toThrow('Failed to perform search')
        expect(mockConsole.error).toHaveBeenCalledWith('Search failed:', apiError)
  ...
  describe('searchPost (POST method)', () => {
    it('should perform POST search with full request body', async () => {
      ...
      expect(mockApiWithTimeout.fast.post).toHaveBeenCalledWith('/search', searchRequest)
      expect(result).toEqual(mockResponse)
    })
    ...
    it('should handle POST search errors and log them', async () => {
      ...
      // Act & Assert
      await expect(service.searchPost(searchRequest)).rejects.toThrow('Failed to perform search')
      expect(mockConsole.error).toHaveBeenCalledWith('Search POST failed:', mockError)
      expect(mockApiWithTimeout.fast.post).toHaveBeenCalledTimes(1)
    })
    ...
    it('should handle large POST request bodies', async () => {
      ...
      // Assert
      expect(mockApiWithTimeout.fast.post).toHaveBeenCalledWith('/search', largeSearchRequest)
      expect(result).toEqual(mockResponse)
    })
    ...
    it('should handle POST network errors', async () => {
      ...
      // Act & Assert
      await expect(service.searchPost(searchRequest)).rejects.toThrow('Failed to perform search')
      expect(mockConsole.error).toHaveBeenCalledWith('Search POST failed:', networkError)
    })
  })
  ...
  describe('service integration', () => {
    ...
    it('should handle mixed successful and failed requests', async () => {
      ...
      await expect(service.search(failRequest)).rejects.toThrow('Failed to perform search')
      expect(mockConsole.error).toHaveBeenCalledWith('Search failed:', expect.any(Error))
    })
  })
  ...
  describe('edge cases', () => {
    ...
    it('should preserve error context in error messages', async () => {
      ...
      // Act & Assert
      await expect(service.search(searchRequest)).rejects.toThrow('Failed to perform search')
      expect(mockConsole.error).toHaveBeenCalledWith('Search failed:', originalError)
    })
  })
})