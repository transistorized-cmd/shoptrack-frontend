...
import api from '../api'
import type { UserSettings, SettingKeyValue, SettingsResponse, SettingUpdateRequest } from '@/types/settings'
...
describe('settingsService', () => {
  ...
  describe('updateSettings', () => {
    it('should update partial settings successfully', async () => {
      ...
      expect(mockApi.put).toHaveBeenCalledWith('/auth/settings', partialSettings)
    ...
    it('should handle update settings API errors', async () => {
      ...
      // Act & Assert
      await expect(settingsService.updateSettings(settings)).rejects.toThrow('Update failed')
    })
  })
  ...
  describe('getSetting', () => {
    it('should get specific setting by key', async () => {
      ...
      // Assert
      expect(mockApi.get).toHaveBeenCalledTimes(1)
      expect(mockApi.get).toHaveBeenCalledWith('/auth/settings/display.currency')
    ...
    it('should handle nested setting keys', async () => {
      ...
      // Act
      const result = await settingsService.getSetting('notifications.receiptProcessed')

      // Assert
      expect(mockApi.get).toHaveBeenCalledWith('/auth/settings/notifications.receiptProcessed')
      expect(result).toEqual(mockSetting)
    })
    ...
    it('should handle get setting API errors', async () => {
      ...
      // Act & Assert
      await expect(settingsService.getSetting('invalid.key')).rejects.toThrow('Setting not found')
    })
  })
  ...
  describe('updateSetting', () => {
    it('should update specific setting by key', async () => {
      ...
      // Act
      const result = await settingsService.updateSetting('display.currency', 'EUR')
    ...
    it('should handle null and undefined values', async () => {
      // Arrange
      const mockResponse: SettingsResponse = { success: true, message: 'Updated' }
      mockApi.put.mockResolvedValue({ data: mockResponse })

      // Act & Assert
      await settingsService.updateSetting('some.key', null)
      expect(mockApi.put).toHaveBeenCalledWith('/auth/settings/some.key', { value: null })

      await settingsService.updateSetting('another.key', undefined)
      expect(mockApi.put).toHaveBeenCalledWith('/auth/settings/another.key', { value: undefined })
    })
  ...
  describe('service integration', () => {
    ...
    it('should handle mixed successful and failed operations', async () => {
      ...
      await expect(settingsService.updateCurrency('INVALID')).rejects.toThrow('Currency update failed')
  ...
  describe('edge cases', () => {
    it('should handle empty string setting keys', async () => {
      // Arrange
      mockApi.get.mockResolvedValue({ data: { key: '', value: null, type: 'unknown' } })
    ...
    it('should handle very long setting keys', async () => {
      ...
      mockApi.put.mockResolvedValue({ data: { success: true, message: 'Updated' } })
    ...
    it('should handle network timeouts gracefully', async () => {
      ...
      // Act & Assert
      await expect(settingsService.getSettings()).rejects.toThrow('Network timeout')
      expect(mockApi.get).toHaveBeenCalledTimes(1)
    })
