...
describe('Categories Store', () => {
  ...
  describe('fetchCategories', () => {
    it('should fetch categories for a single locale', async () => {
      vi.mocked(categoriesService.getCategories).mockResolvedValue(mockCategoriesEN);
    ...
    it('should handle fetch errors', async () => {
      ...
      vi.mocked(categoriesService.getCategories).mockRejectedValue(new Error(errorMessage));
    ...
    it('should handle non-Error rejections', async () => {
      vi.mocked(categoriesService.getCategories).mockRejectedValue('String error');
    ...
    it('should preserve existing locales when adding new ones', async () => {
      ...
      // Second fetch
      vi.mocked(categoriesService.getCategories).mockResolvedValue(mockCategoriesES);
    ...
    it('should override existing locale data', async () => {
      ...
      const updatedCategories = [{ ...mockCategoriesEN[0], name: 'Updated Food' }];
      vi.mocked(categoriesService.getCategories).mockResolvedValue(updatedCategories);
      await store.fetchCategories('en');
  ...
  describe('fetchAllLocales', () => {
    it('should fetch categories for multiple locales', async () => {
      vi.mocked(categoriesService.getCategories)
  ...
  describe('Auto Refresh', () => {
    ...
    it('should handle refresh errors gracefully', async () => {
      ...
      vi.mocked(categoriesService.getCategories).mockRejectedValue(new Error('Refresh failed'));
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle very large category datasets', async () => {
      ...
      vi.mocked(categoriesService.getCategories).mockResolvedValue(largeDataset);
    ...
    it('should handle categories with special characters', async () => {
      ...
      vi.mocked(categoriesService.getCategories).mockResolvedValue(specialCategories);
    ...
    it('should handle null and undefined category properties', async () => {
      ...
      vi.mocked(categoriesService.getCategories).mockResolvedValue(categoriesWithNulls);
  ...
  describe('Performance', () => {
    ...
    it('should handle memory efficiently with large locale count', async () => {
      ...
      vi.mocked(categoriesService.getCategories).mockImplementation((locale) => {
        return Promise.resolve([{
          id: 1,
          key: 'test',
          name: `Test in ${locale}`,
