...
describe('useReportsStore', () => {
  ...
  describe('generateReport', () => {
    it('should generate report successfully', async () => {
      ...
      expect(mockReportsService.generateReport).toHaveBeenCalledWith(mockRequest)
    ...
    it('should handle report generation errors', async () => {
      ...
      // Act & Assert
      await expect(store.generateReport(mockRequest)).rejects.toThrow('Report generation failed')

      expect(store.loading).toBe(false)
      expect(store.error).toBe('Report generation failed')
      expect(store.currentReport).toBeNull()
      expect(mockConsole.error).toHaveBeenCalledWith('Error generating report:', mockError)
    ...
    it('should handle non-Error objects', async () => {
      ...
      mockReportsService.generateReport.mockRejectedValue('String error message')

      // Act & Assert
      await expect(store.generateReport(mockRequest)).rejects.toThrow('String error message')

      expect(store.error).toBe('Failed to generate report')
      expect(mockConsole.error).toHaveBeenCalledWith('Error generating report:', 'String error message')
    })
  ...
  describe('exportReport', () => {
    it('should export report as PDF successfully', async () => {
      ...
      expect(mockReportsService.exportReport).toHaveBeenCalledWith(mockRequest, 'pdf')
      expect(store.loading).toBe(false)
      expect(store.error).toBeNull()
    ...
    it('should export report in different formats', async () => {
      ...
      for (const { format, mimeType } of formats) {
        ...
        // Assert
        expect(mockReportsService.exportReport).toHaveBeenCalledWith(mockRequest, format)

        const mockLink = mockCreateElement.mock.results[mockCreateElement.mock.results.length - 1].value
    ...
    it('should handle export errors', async () => {
      ...
      // Act & Assert
      await expect(store.exportReport(mockRequest, 'pdf')).rejects.toThrow('Export failed')

      expect(store.loading).toBe(false)
      expect(store.error).toBe('Export failed')
      expect(mockConsole.error).toHaveBeenCalledWith('Error exporting report:', mockError)
    ...
    it('should handle non-Error objects in export', async () => {
      ...
      // Act & Assert
      await expect(store.exportReport(mockRequest, 'pdf')).rejects.toThrow('Export string error')

      expect(store.error).toBe('Failed to export report')
      expect(mockConsole.error).toHaveBeenCalledWith('Error exporting report:', 'Export string error')
    })
  ...
  describe('store integration', () => {
    ...
    it('should maintain state consistency across operations', async () => {
      ...
      // Failed generation
      await expect(store.generateReport(failRequest)).rejects.toThrow('Generation failed')
  ...
  describe('edge cases', () => {
    ...
    it('should handle DOM operations failure gracefully', async () => {
      ...
      // Act & Assert
      await expect(store.exportReport(mockRequest, 'pdf')).rejects.toThrow('DOM operation failed')
