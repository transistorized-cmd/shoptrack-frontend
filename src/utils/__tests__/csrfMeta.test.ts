...
describe('csrfMeta utilities', () => {
  ...
  describe('removeCsrfTokenMeta', () => {
    ...
    it('should remove only first csrf-token meta tag if multiple exist', () => {
      ...
      // Assert
      const remainingCsrfTags = document.querySelectorAll('meta[name="csrf-token"]')
  ...
  describe('getCsrfTokenWithFallback', () => {
    ...
    it('should fallback to meta tag when composable fails', async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith('Failed to get CSRF token from composable:', expect.any(Error))
    })
    ...
    it('should return null when both sources fail', async () => {
      ...
      expect(result).toBeNull()
      expect(consoleWarnSpy).toHaveBeenCalledWith('Failed to get CSRF token from composable:', expect.any(Error))
      expect(consoleErrorSpy).toHaveBeenCalledWith('No valid CSRF token found in any source')
    })
  ...
  describe('integration tests', () => {
    it('should work with full create-update-remove cycle', () => {
      ...
      expect(document.querySelectorAll('meta[name="csrf-token"]')).toHaveLength(1)
