...
describe("currencyFormat", () => {
  ...
  describe("formatCurrency", () => {
    describe("when receipt currency matches user preference", () => {
      it("should show only symbol for USD", () => {
        expect(formatCurrency(20.0, "USD", "USD")).toBe("$20.00");
      });

      it("should show only symbol for EUR", () => {
        expect(formatCurrency(20.0, "EUR", "EUR")).toBe("€20.00");
      });

      it("should show only symbol when no receipt currency provided", () => {
        expect(formatCurrency(20.0, undefined, "EUR")).toBe("€20.00");
      });
    });

    describe("when receipt currency differs from user preference", () => {
      it("should show only symbol for unique symbols", () => {
        expect(formatCurrency(20.0, "EUR", "USD")).toBe("€20.00");
        expect(formatCurrency(20.0, "GBP", "USD")).toBe("£20.00");
        expect(formatCurrency(20.0, "INR", "USD")).toBe("₹20.00");
      });

      it("should add currency code for USD when user preference is different", () => {
        expect(formatCurrency(20.0, "USD", "EUR")).toBe("$20.00 USD");
      });

      it("should add currency code for MXN when user preference is different", () => {
        expect(formatCurrency(20.0, "MXN", "EUR")).toBe("$20.00 MXN");
      });

      it("should add currency code for shared ¥ symbol", () => {
        expect(formatCurrency(20.0, "JPY", "USD")).toBe("¥20.00 JPY");
        expect(formatCurrency(20.0, "CNY", "USD")).toBe("¥20.00 CNY");
      });
    });

    describe("formatting options", () => {
      it("should respect decimal places option", () => {
        expect(formatCurrency(20.123, "USD", "USD", { decimals: 0 })).toBe(
          "$20",
        );
        expect(formatCurrency(20.123, "USD", "USD", { decimals: 3 })).toBe(
          "$20.123",
        );
      });
      ...
      it("should handle showZeroDecimals option", () => {
        expect(
          formatCurrency(20, "USD", "USD", { showZeroDecimals: false }),
        ).toBe("$20");
        expect(
          formatCurrency(20, "USD", "USD", { showZeroDecimals: true }),
        ).toBe("$20.00");
        expect(
          formatCurrency(20.5, "USD", "USD", { showZeroDecimals: false }),
        ).toBe("$20.50");
      });
    });
    ...
    describe("fallback for unknown currencies", () => {
      it("should handle unknown currency codes", () => {
        expect(formatCurrency(20.0, "XYZ", "USD")).toBe("XYZ 20.00");
      });
    });
  ...
  describe("formatCurrencyCompact", () => {
    it("should use default compact formatting", () => {
      expect(formatCurrencyCompact(20.0, "USD", "EUR")).toBe("$20.00 USD");
      expect(formatCurrencyCompact(20.0, "EUR", "USD")).toBe("€20.00");
      expect(formatCurrencyCompact(20.0, "USD", "USD")).toBe("$20.00");
    });
  });
  ...
  describe("real-world examples from requirements", () => {
    it("should match the examples from requirements", () => {
      ...
      expect(formatCurrency(20.0, "MXN", "USD")).toBe("$20.00 MXN");
    });
    ...
    it("should handle additional scenarios", () => {
      ...
      expect(formatCurrency(20.0, "GBP", "EUR")).toBe("£20.00");
