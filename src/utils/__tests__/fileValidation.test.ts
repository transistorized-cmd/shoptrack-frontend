...
describe("File Validation Utility", () => {
  ...
  describe("validateFile", () => {
    describe("Basic Validation", () => {
      ...
      it("should accept valid files", async () => {
        ...
        const file = createFileWithSignature(
          "test.jpg",
          [0xff, 0xd8, 0xff],
          "image/jpeg",
        );
    ...
    describe("File Size Validation", () => {
      ...
      it("should reject files exceeding custom size limit", async () => {
        ...
        expect(result.errors[0]).toContain(
          "File size (6MB) exceeds maximum allowed size (5MB)",
        );
      });
    });
        ...
        for (const file of dangerousFiles) {
          ...
          expect(
            result.errors.some((e) =>
              e.includes("Dangerous file type detected"),
            ),
          ).toBe(true);
        }
      });
    ...
    describe("MIME Type Validation", () => {
      it("should warn about MIME type mismatches in strict mode", async () => {
        ...
        expect(
          result.warnings.some((w) =>
            w.includes("MIME type (image/png) doesn't match"),
          ),
        ).toBe(true);
      });
      ...
      it("should accept correct MIME types", async () => {
        ...
        expect(
          result.warnings.filter((w) => w.includes("MIME type")).length,
        ).toBe(0);
      });
    });
    ...
    describe("File Signature Validation", () => {
      ...
      it("should detect invalid file signatures", async () => {
        ...
        expect(result.isValid).toBe(false);
        expect(result.errors[0]).toContain(
          "File content doesn't match file extension",
        );
      });
    ...
    describe("Suspicious Pattern Detection", () => {
      it("should detect URL-encoded characters", async () => {
        ...
        expect(result.warnings).toContain(
          "Filename contains URL-encoded characters",
        );
      });
    ...
    describe("File Type Detection", () => {
      it("should correctly detect file types", async () => {
        const testCases = [
          ...
          {
            file: createMockFile("test.unknown"),
  ...
  describe("Edge Cases and Error Conditions", () => {
    it("should handle files without extensions", async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(
        result.errors.some((e) => e.includes("File type not allowed:")),
      ).toBe(true);
    });
