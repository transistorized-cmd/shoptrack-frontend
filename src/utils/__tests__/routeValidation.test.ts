...
describe("routeValidation utilities", () => {
  ...
  describe("validateNumericId", () => {
    ...
    describe("invalid numeric IDs", () => {
      ...
      it("should handle very large numbers", () => {
        ...
        // Act
        const result = validateNumericId(veryLargeNumber, {
  ...
  describe("validateStringParam", () => {
    describe("valid strings", () => {
      ...
      it("should respect length constraints", () => {
        ...
        expect(validateStringParam("verylongstring", options).isValid).toBe(
          false,
        );
      });
    });
    ...
    describe("invalid strings", () => {
      ...
      it("should reject strings with dangerous characters", () => {
        ...
        dangerousStrings.forEach((str) => {
          ...
          expect(result.error).toBe(
            "String parameter contains invalid characters",
          );
        });
      });
      ...
      it("should reject strings that do not match allowed pattern", () => {
        ...
        // Act & Assert
        expect(validateStringParam("valid-name_123", options).isValid).toBe(
          true,
        );
        expect(validateStringParam("invalid@name", options).isValid).toBe(
          false,
        );
        expect(validateStringParam("invalid name", options).isValid).toBe(
          false,
  ...
  describe("sanitization functions", () => {
    ...
    describe("sanitizeItemName", () => {
      ...
      it("should remove dangerous script tags", () => {
        // Act & Assert
        expect(sanitizeItemName("Product<script>alert(1)</script>Name")).toBe(
          "ProductName",
        );
        expect(
          sanitizeItemName('Item<iframe src="evil"></iframe>Description'),
      ...
      it("should remove event handlers", () => {
        // Act & Assert
        expect(sanitizeItemName("Product onclick=alert(1) Name")).toBe(
          "Product Name",
        );
        expect(sanitizeItemName("Item onmouseover=evil() Description")).toBe(
          "Item Description",
        );
      });
    ...
    describe("sanitizeText", () => {
      it("should remove all dangerous HTML tags", () => {
        // Act & Assert
        expect(sanitizeText("Text<script>alert(1)</script>More")).toBe(
          "TextMore",
        );
        expect(sanitizeText('Text<iframe src="bad"></iframe>More')).toBe(
          "TextMore",
        );
        expect(sanitizeText('Text<object data="bad"></object>More')).toBe(
          "TextMore",
        );
      ...
      it("should remove dangerous protocols", () => {
        ...
        expect(sanitizeText('Run vbscript:msgbox("bad") code')).toBe(
          "Run code",
        );
      });
      ...
      it("should preserve legitimate text formatting", () => {
        ...
        const legitimateText =
          "This is a normal sentence with punctuation! And numbers: 123.";
  ...
  describe("security and edge cases", () => {
    ...
    it("should handle XML and data URL injections", () => {
      expect(
        validateStringParam('<?xml version="1.0"?><root>evil</root>').isValid,
      ).toBe(false);
      expect(
        validateStringParam("data:text/html,<script>alert(1)</script>").isValid,
      ).toBe(false);
    });

    it("should handle various injection attempts", () => {
      // Test each injection attempt individually so we can see which ones pass
      expect(validateStringParam("1'; DROP TABLE users; --").isValid).toBe(
        false,
      );
      expect(validateStringParam("1 OR 1=1").isValid).toBe(false);
      expect(validateStringParam("1 UNION SELECT * FROM secrets").isValid).toBe(
        false,
      );
      expect(
        validateStringParam('1<script>alert("XSS")</script>').isValid,
      ).toBe(false);
      ...
      expect(validateStringParam("\\\\server\\share\\file").isValid).toBe(
        false,
      );
      expect(
        validateStringParam("javascript:alert(document.cookie)").isValid,
      ).toBe(false);
      ...
      expect(validateStringParam("') OR ('1'='1").isValid).toBe(false);
      expect(validateStringParam("1%27%20OR%20%271%27%3D%271").isValid).toBe(
        false,
      );
      expect(validateStringParam("1' OR 1=1--").isValid).toBe(false);
      expect(validateStringParam("1' OR 1=1#").isValid).toBe(false);
