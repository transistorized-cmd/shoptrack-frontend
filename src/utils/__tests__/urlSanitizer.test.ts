...
describe('urlSanitizer utilities', () => {
  ...
  describe('sanitizeImageUrl', () => {
    ...
    describe('dangerous URLs', () => {
      it('should reject javascript: URLs', () => {
        ...
        dangerousUrls.forEach(url => {
          expect(sanitizeImageUrl(url, fallback)).toBe(fallback)
          expect(consoleWarnSpy).toHaveBeenCalledWith('Dangerous URL pattern detected:', expect.any(String))
        })
      })
    ...
    describe('private/local URLs', () => {
      it('should reject localhost URLs', () => {
        ...
        localhostUrls.forEach(url => {
          expect(sanitizeImageUrl(url)).toContain('data:image/svg+xml')
          expect(consoleWarnSpy).toHaveBeenCalledWith('Private/local URL detected:', expect.any(String))
        })
      })
    ...
    describe('invalid URLs and edge cases', () => {
      it('should return fallback for null/undefined URLs', () => {
        ...
        expect(sanitizeImageUrl(undefined, customFallback)).toBe(customFallback)
      })
      ...
      it('should reject unsupported protocols', () => {
        ...
        unsupportedProtocols.forEach(url => {
          expect(sanitizeImageUrl(url, fallback)).toBe(fallback)
          expect(consoleWarnSpy).toHaveBeenCalledWith('Invalid URL scheme for image:', expect.any(String))
        })
      })
    })
  ...
  describe('useSafeImageUrl composable', () => {
    ...
    it('should work the same as direct function calls', () => {
      ...
      expect(composableSafeLinkUrl(testLinkUrl)).toBe(sanitizeLinkUrl(testLinkUrl))
    })
  })
  ...
  describe('comprehensive security tests', () => {
    ...
    it('should handle various URL encoding attacks', () => {
      ...
      encodedAttacks.forEach(url => {
        ...
        expect(imageResult === url || imageResult.includes('data:image/svg+xml')).toBe(true)
    ...
    it('should consistently handle boundary conditions', () => {
      ...
      const boundaryTests = [
        { input: null, expected: expect.stringContaining('data:image/svg+xml') },
        { input: undefined, expected: expect.stringContaining('data:image/svg+xml') },
        { input: '', expected: expect.stringContaining('data:image/svg+xml') },
        { input: '   ', expected: expect.stringContaining('data:image/svg+xml') },
        { input: 'https://example.com/valid.jpg', expected: 'https://example.com/valid.jpg' }
      ]
