...
describe('CategoryAnalytics Component - Locale Switching', () => {
  ...
  const createWrapper = (props = {}, locale = 'en', routeQuery = {}) => {
    const { mockRouter: router, mockRoute } = createMockRouter('/analytics/categories');
    mockRouter = router;
  ...
  beforeEach(async () => {
    ...
    mockCategoriesStore.getName.mockImplementation((id: number, locale: string) => {
  ...
  describe('Locale Initialization', () => {
    ...
    it('should include locale parameter in API requests', async () => {
      ...
      const categoryApiCall = apiCalls.find(call => call[0].includes('/api/analytics/categories?'));
    ...
    it('should handle different locales correctly', async () => {
      ...
      for (const locale of locales) {
        ...
        expect(mockCategoriesStore.fetchCategories).toHaveBeenCalledWith(locale);
  ...
  describe('API Request Locale Parameters', () => {
    it('should include locale in category data requests', async () => {
      ...
      const categoryApiCall = mockedAxios.get.mock.calls.find(
        call => call[0].includes('/api/analytics/categories?')
      );
    ...
    it('should include locale in item data requests', async () => {
      ...
      const itemApiCall = mockedAxios.get.mock.calls.find(
        call => call[0].includes('/api/analytics/categories/items?')
      );
    ...
    it('should include locale in receipt data requests', async () => {
      ...
      const receiptApiCall = mockedAxios.get.mock.calls.find(
        call => call[0].includes('/api/analytics/categories/items/receipts?')
      );
