...
describe("Home View", () => {
  ...
  const createWrapper = (
    ...
    mockReceiptsStore = {
      ...
      pendingReceipts: receiptData.filter(
        (r) => r.processingStatus === "pending",
      ),
      completedReceipts: receiptData.filter(
  ...
  describe("Stats Display", () => {
    it("should display correct total receipts count", () => {
      ...
      const totalReceiptsCard =
        wrapper.find('[data-testid="total-receipts"]') ||
        wrapper
          .findAll(".card")
          .find((card) => card.text().includes("Total Receipts"));
    ...
    it("should display correct pending receipts count", () => {
      ...
      const pendingCard = wrapper
        .findAll(".card")
        .find((card) => card.text().includes("Pending"));
    ...
    it("should display correct completed receipts count", () => {
      ...
      const completedCard = wrapper
        .findAll(".card")
        .find((card) => card.text().includes("Completed"));
    ...
    it("should display correct plugins count", () => {
      ...
      const pluginsCard = wrapper
        .findAll(".card")
        .find((card) => card.text().includes("Available Plugins"));
  ...
  describe("Loading States", () => {
    ...
    it("should hide stats when loading", () => {
      ...
      const hasStatsGrid = wrapper
        .find(".grid.grid-cols-1.gap-5.sm\\:grid-cols-2.lg\\:grid-cols-4")
        .exists();
  ...
  describe("Recent Receipts Section", () => {
    ...
    it("should format receipt dates correctly", () => {
      ...
      const dateRegex =
        /\d{1,2}\/\d{1,2}\/\d{4}|\d{1,2}\.\d{1,2}\.\d{4}|\d{4}-\d{2}-\d{2}/;
  ...
  describe("Navigation Links", () => {
    ...
    it("should have view all link in recent receipts section", () => {
      ...
      const recentReceiptsSection = wrapper
        .findAll(".card")
        .find((card) => card.text().includes("Recent Receipts"));
    ...
    it("should have individual receipt view links", () => {
      ...
      const receiptLinks = wrapper
        .findAll("a")
        .filter((link) => link.attributes("href")?.startsWith("/receipts/"));
  ...
  describe("Status Methods", () => {
    ...
    it("should return correct status colors", () => {
      ...
      expect(component.getStatusColor("processing")).toBe(
        "bg-blue-100 text-blue-600",
      );
      expect(component.getStatusColor("failed")).toBe(
  ...
  describe("Component Lifecycle", () => {
    ...
    it("should handle concurrent data fetching", async () => {
      ...
      mockPluginsStore.fetchAllPlugins = vi
        .fn()
        .mockImplementation(
    ...
    it("should handle data fetching errors gracefully", async () => {
      ...
      mockPluginsStore.fetchAllPlugins = vi
        .fn()
        .mockRejectedValue(new Error("Plugin fetch failed"));
  ...
  describe("Responsive Design", () => {
    it("should have responsive grid classes", () => {
      ...
      const actionsGrid = wrapper.find(
