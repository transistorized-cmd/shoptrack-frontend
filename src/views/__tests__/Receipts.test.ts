...
vi.mock('@/components/common/LocalizedDateInput.vue', () => ({
  default: {
    ...
    template: '<input :id="id" :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" @change="$emit(\'change\')" />',
    props: ['id', 'modelValue'],
...
vi.mock('@/components/receipts/ReceiptCard.vue', () => ({
  default: {
    name: 'ReceiptCard',
    template: '<div class="receipt-card" data-testid="receipt-card">{{ receipt.filename }}</div>',
    props: ['receipt'],
    emits: ['delete']
  }
}));
...
describe('Receipts View', () => {
  ...
  const createWrapper = (receiptData = mockReceipts, loading = false, error = null) => {
    // Setup store mock
    mockReceiptsStore = {
  ...
  describe('Pagination', () => {
    ...
    it('should disable next button on last page', () => {
      ...
      // Check component logic for disabled state
      const component = wrapper.vm as any;
      expect(mockReceiptsStore.pagination.page >= mockReceiptsStore.pagination.totalPages).toBe(true);
    });
  ...
  describe('Receipt Management', () => {
    it('should handle receipt deletion with confirmation', async () => {
      ...
      expect(global.confirm).toHaveBeenCalledWith('Are you sure you want to delete this receipt?');
    ...
    it('should handle deletion errors gracefully', async () => {
      global.confirm = vi.fn().mockReturnValue(true);
      mockReceiptsStore.deleteReceipt = vi.fn().mockRejectedValue(new Error('Delete failed'));
  ...
  describe('Responsive Design', () => {
    it('should have responsive grid classes for receipts', () => {
      ...
      const receiptsGrid = wrapper.find('.grid.grid-cols-1.gap-4.sm\\:gap-6.sm\\:grid-cols-2.lg\\:grid-cols-3');
    ...
    it('should have responsive grid classes for filters', () => {
      ...
      const filtersGrid = wrapper.find('.grid.grid-cols-1.gap-4.sm\\:grid-cols-2.lg\\:grid-cols-4');
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle store errors during filter changes', async () => {
      mockReceiptsStore.fetchReceipts = vi.fn().mockRejectedValue(new Error('Network error'));
      wrapper = createWrapper();
  ...
  describe('Performance', () => {
    ...
    it('should avoid unnecessary re-renders', async () => {
      ...
      const initialRenderCount = mockReceiptsStore.fetchReceipts.mock.calls.length;
      ...
      expect(mockReceiptsStore.fetchReceipts.mock.calls.length).toBeGreaterThanOrEqual(initialRenderCount);
    });
  });
});