...
describe("Upload View", () => {
  ...
  beforeEach(async () => {
    ...
    mockGetPluginTranslations = vi.mocked(
      pluginI18nModule.getPluginTranslations,
    );
    mockHasPluginTranslations = vi.mocked(
      pluginI18nModule.hasPluginTranslations,
    );
  ...
  describe("Component Rendering", () => {
    ...
    it("should not render plugin grid when no plugins available", async () => {
      ...
      expect(wrapper.find(".grid.grid-cols-1.lg\\:grid-cols-2").exists()).toBe(
        false,
      );
  ...
  describe("Plugin Display", () => {
    ...
    it("should display plugin capabilities correctly", async () => {
      ...
      expect(capabilities.findAll(".flex.items-center").length).toBeGreaterThan(
        3,
      );
    });
  ...
  describe("File Upload Handling", () => {
    ...
    it("should validate file before accepting selection", async () => {
      ...
      expect(wrapper.vm.fileValidationResults[plugin.key].errors).toEqual([
        "File too large",
      ]);
    });
    ...
    it("should show validation warnings when file has warnings", async () => {
      ...
      expect(wrapper.vm.fileValidationResults[plugin.key].warnings).toEqual([
  ...
  describe("File Upload Process", () => {
    ...
    it("should handle upload errors gracefully", async () => {
      ...
      expect(wrapper.vm.uploadResult?.errors).toEqual([
  ...
  describe("Plugin Translation", () => {
    it("should use plugin translations when available", () => {
      ...
      expect(mockGetPluginTranslations).toHaveBeenCalledWith(
        "test-plugin",
        "en",
      );
    });
    ...
    it("should fallback to key when no translations available", () => {
      ...
      const result = wrapper.vm.getPluginTranslation(
        "test-plugin",
        "actions.upload",
      );
  ...
  describe("File Type Mapping", () => {
    it("should create file type map from available plugins", async () => {
      ...
      expect(fileTypeMap.pdf).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ key: "pdf-plugin" }),
        ]),
      );
      expect(fileTypeMap.jpg).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ key: "pdf-plugin" }),
        ]),
      );
      expect(fileTypeMap.csv).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ key: "csv-plugin" }),
        ]),
      );
      expect(fileTypeMap.xlsx).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ key: "csv-plugin" }),
        ]),
      );
  ...
  describe("Plugin Loading", () => {
    ...
    it("should handle plugin loading errors gracefully", async () => {
      mockPluginsStore.fetchAllPlugins.mockRejectedValue(
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "Failed to load plugins, using demo data:",
        expect.any(Error),
      );
