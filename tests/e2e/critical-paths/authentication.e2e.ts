...
test.describe("Authentication Critical Paths", () => {
  ...
  test.describe("User Registration Flow", () => {
    ...
    test("should handle duplicate email registration", async ({ page }) => {
      ...
      await page.waitForSelector(
        '[data-testid="registration-error"], [data-testid="email-exists-error"]',
      );
      ...
      expect(errorMessage).toMatch(
        /email.*already.*exists|already.*registered/i,
      );
    });
  });
  ...
  test.describe("User Login Flow", () => {
    test("should complete successful login", async ({ page }) => {
      ...
      console.log(
        `✅ Login completed in ${loginTime}ms, page loaded in ${pageLoadTime}ms`,
      );
    });
    ...
    test("should handle account lockout after multiple failures", async ({
      page,
    }) => {
      await authPage.goto();
  ...
  test.describe("Password Recovery", () => {
    ...
    test("should validate password reset form", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="password-error"]'),
      ).toContainText(/requirements|strong/i);
