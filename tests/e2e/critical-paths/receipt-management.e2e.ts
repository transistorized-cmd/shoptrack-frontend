...
test.describe("Receipt Management and Search Critical Paths", () => {
  ...
  test.describe("Receipt List and Navigation", () => {
    test("should display receipts list with proper formatting", async ({
      page,
    }) => {
      ...
      for (let i = 0; i < sampleReceipts.length; i++) {
        ...
        const receiptElement = page
          .locator('[data-testid="receipt-item"]')
          .nth(i);

        await expectElementText(
          page,
          `[data-testid="receipt-merchant-${i}"]`,
          receipt.merchant,
        );
        await expectElementText(
          page,
          `[data-testid="receipt-total-${i}"]`,
          receipt.total.toString(),
        );
        await expectElementText(
          page,
          `[data-testid="receipt-date-${i}"]`,
          receipt.date,
        );
        await expectElementText(
          page,
          `[data-testid="receipt-category-${i}"]`,
          receipt.category,
        );
      }
    ...
    test("should navigate to receipt detail page", async ({ page }) => {
      ...
      await expectElementText(
        page,
        '[data-testid="detail-merchant"]',
        firstReceipt.merchant,
      );
      await expectElementText(
        page,
      ...
      await expectElementCount(
        page,
        '[data-testid="receipt-item-line"]',
        firstReceipt.items.length,
      );
    ...
    test("should handle empty receipt list gracefully", async ({ page }) => {
      ...
      await expectElementText(
        ...
        /no receipts found|upload your first receipt/i,
      );
  ...
  test.describe("Search Functionality", () => {
    ...
    test("should search receipts by total amount range", async ({ page }) => {
      ...
      const minAmount = 50;
      const maxAmount = 150;
      const expectedResults = sampleReceipts.filter(
        (r) => r.total >= minAmount && r.total <= maxAmount,
      );

      await mockApiResponse(
        page,
        `**/api/receipts/search?minAmount=${minAmount}&maxAmount=${maxAmount}*`,
        {
          receipts: expectedResults,
          totalCount: expectedResults.length,
    ...
    test("should search receipts by tags", async ({ page }) => {
      ...
      await expectElementVisible(
        page,
        `[data-testid="active-tag-${searchTag}"]`,
      );
    });
    ...
    test("should handle no search results gracefully", async ({ page }) => {
      ...
      await mockApiResponse(
        page,
        `**/api/receipts/search?q=${noResultsSearch}*`,
        {
          receipts: [],
          totalCount: 0,
    ...
    test("should provide search autocomplete and suggestions", async ({
      page,
    }) => {
      await receiptsPage.goto();
  ...
  test.describe("Filtering and Sorting", () => {
    ...
    test("should sort receipts by different criteria", async ({ page }) => {
      ...
      const sortedByAmount = [...sampleReceipts].sort(
        (a, b) => b.total - a.total,
      );
      ...
      const sortedByDate = [...sampleReceipts].sort(
    ...
    test("should combine multiple filters effectively", async ({ page }) => {
      ...
      await mockApiResponse(
        page,
        `**/api/receipts?category=${category}&minAmount=${minAmount}&tags=${tag}*`,
        {
          receipts: expectedResults,
          totalCount: expectedResults.length,
  ...
  test.describe("Receipt Actions and Management", () => {
    test("should delete receipt with confirmation", async ({ page }) => {
      ...
      await mockApiResponse(
        page,
        `**/api/receipts/${receiptToDelete.id}`,
        {
          success: true,
      ...
      const merchantElements = page.locator(
    ...
    test("should duplicate receipt", async ({ page }) => {
      ...
      await mockApiResponse(
        page,
        `**/api/receipts/${originalReceipt.id}`,
        originalReceipt,
      );
      await mockApiResponse(
        page,
      ...
      await expectElementText(
        page,
        '[data-testid="detail-merchant"]',
        duplicatedReceipt.merchant,
      );
      await expectElementText(
        page,
        '[data-testid="detail-total"]',
        `$${duplicatedReceipt.total}`,
      );
      ...
      await expectElementText(
        page,
        '[data-testid="original-receipt-link"]',
        originalReceipt.id,
      );
    });
  ...
  test.describe("Bulk Actions", () => {
    test("should select multiple receipts and perform bulk actions", async ({
      page,
    }) => {
      await receiptsPage.goto();
      ...
      expect(bulkDownload.suggestedFilename()).toMatch(
        /selected.*receipts.*\.pdf$/i,
      );
    });

    test("should select all receipts with select all checkbox", async ({
      page,
    }) => {
      await receiptsPage.goto();
      ...
      const allCheckboxes = page.locator(
        '[data-testid^="select-receipt-"]:checked',
      );
    ...
    test("should handle bulk delete with confirmation", async ({ page }) => {
      ...
      await expectElementText(
        page,
        '[data-testid="bulk-delete-message"]',
  ...
  test.describe("Pagination and Performance", () => {
    ...
    test("should handle loading states properly", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="loading-receipts"]'),
      ).not.toBeVisible();
    });
  });
});
