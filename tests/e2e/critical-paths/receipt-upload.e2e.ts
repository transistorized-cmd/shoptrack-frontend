...
test.describe('Receipt Upload and Processing Critical Paths', () => {
  ...
  test.describe('Single File Upload', () => {
    test('should upload single receipt image successfully', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="processed-merchant"]')).toContainText('Grocery Store');
      await expect(page.locator('[data-testid="processed-total"]')).toContainText('45.67');
    ...
    test('should handle large file uploads', async ({ page }) => {
      ...
      while (progressVisible) {
        try {
          await page.waitForSelector('[data-testid="upload-progress"]', { timeout: 500 });
    ...
    test('should validate file types and sizes', async ({ page }) => {
      ...
      const sizeErrorMessage = await page.locator('[data-testid="upload-error"]').textContent();
  ...
  test.describe('Multiple File Upload', () => {
    test('should upload multiple receipts simultaneously', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="all-uploads-complete"]', { timeout: 60000 });
      const totalUploadTime = Date.now() - startTime;

      // Verify all files were processed
      const successElements = page.locator('[data-testid="upload-success-item"]');
      ...
      console.log(`✅ Multiple file upload (${files.length} files) completed in ${totalUploadTime}ms`);
    });
    ...
    test('should handle partial failures in batch upload', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="batch-upload-complete"]', { timeout: 60000 });

      // Verify partial success
      const successElements = page.locator('[data-testid="upload-success-item"]');
      const failureElements = page.locator('[data-testid="upload-failure-item"]');
      ...
      await expect(failureElements.first()).toContainText(/processing.*failed|error/i);

      // Verify retry option for failed uploads
      await expect(page.locator('[data-testid="retry-failed-uploads"]')).toBeVisible();
    });

    test('should show upload queue and progress for multiple files', async ({ page }) => {
      await uploadPage.goto();
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        await expect(queueItems.nth(i).locator('[data-testid="item-status"]'))
      ...
      await expect(page.locator('[data-testid="overall-progress"]')).toContainText('100%');
    });
  });
  ...
  test.describe('Drag and Drop Upload', () => {
    test('should handle drag and drop file upload', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="processed-merchant"]')).toContainText('Drag Store');
    });
    ...
    test('should show drag feedback and validation', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="dropzone-error"]')).toContainText(/invalid.*file.*type/i);
    });
  });
  ...
  test.describe('Receipt Processing and OCR', () => {
    test('should process receipt data accurately', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="processed-merchant"]')).toContainText(expectedData.merchant);
      await expect(page.locator('[data-testid="processed-total"]')).toContainText(expectedData.total.toString());
      await expect(page.locator('[data-testid="processed-date"]')).toContainText('2024-01-15');
      await expect(page.locator('[data-testid="processed-category"]')).toContainText(expectedData.category);
      ...
      for (let i = 0; i < expectedData.items.length; i++) {
        ...
        await expect(item.locator('[data-testid="item-name"]')).toContainText(expectedData.items[i].name);
        await expect(item.locator('[data-testid="item-price"]')).toContainText(expectedData.items[i].price.toString());
      }

      // Verify confidence score
      await expect(page.locator('[data-testid="confidence-score"]')).toContainText('95.6%');

      // Allow manual corrections
      await expect(page.locator('[data-testid="edit-receipt"]')).toBeVisible();
    });

    test('should handle low-quality images with reduced confidence', async ({ page }) => {
      await uploadPage.goto();
      ...
      await mockApiResponse(page, '**/api/receipts/upload', {
        ...
        processedData: {
          ...
          total: 0.00, // Could not read total
          date: null,
          items: [],
      ...
      await expect(page.locator('[data-testid="confidence-score"]')).toContainText('34.2%');

      // Should prompt for manual review
      await expect(page.locator('[data-testid="needs-review"]')).toBeVisible();
      await expect(page.locator('[data-testid="manual-entry-prompt"]')).toBeVisible();
    ...
    test('should allow manual editing of processed data', async ({ page }) => {
      ...
      // Edit item
      await page.fill('[data-testid="edit-item-name-0"]', 'Corrected Item Name');
      ...
      await expect(page.locator('[data-testid="processed-merchant"]')).toContainText('Corrected Store Name');
      await expect(page.locator('[data-testid="processed-total"]')).toContainText('75.50');
    });
  });
  ...
  test.describe('Upload Error Handling', () => {
    ...
    test('should handle server errors during processing', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="try-later-option"]')).toBeVisible();
    });
    ...
    test('should handle timeout during long processing', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="upload-error"], [data-testid="upload-timeout"]',
        { timeout: 40000 });

      const errorOrTimeout = await page.locator('[data-testid="upload-error"], [data-testid="upload-timeout"]').textContent();
      expect(errorOrTimeout).toMatch(/timeout|taking.*longer|try.*again/i);
    });
  });

  test.describe('Integration with Receipt Management', () => {
    test('should seamlessly transition from upload to receipt management', async ({ page }) => {
      await uploadPage.goto();
      ...
      await expect(page.locator('[data-testid="receipt-merchant"]')).toContainText('Integration Store');
      await expect(page.locator('[data-testid="receipt-total"]')).toContainText('89.99');
      ...
      const uploadedReceipt = page.locator('[data-testid="receipt-item"]').filter({
        hasText: 'Integration Store'
      });
      await expect(uploadedReceipt).toBeVisible();
    });

    test('should update dashboard after successful upload', async ({ page }) => {
