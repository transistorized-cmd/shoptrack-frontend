...
test.describe("Reporting and Analytics Critical Paths", () => {
  ...
  const sampleAnalyticsData = {
    overview: {
      ...
      trends: {
        weeklySpending: [120.5, 89.3, 156.78, 203.45, 78.9, 134.67, 198.23],
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await mockApiResponse(
      ...
      sampleAnalyticsData.overview,
    );
    await mockApiResponse(
      page,
      "**/api/analytics/charts*",
      sampleAnalyticsData.charts,
    );
    await mockApiResponse(
      page,
  ...
  test.describe("Dashboard Analytics Overview", () => {
    ...
    test("should display category breakdown accurately", async ({ page }) => {
      ...
      for (let i = 0; i < categories.length; i++) {
        ...
        await expectElementVisible(
          page,
          `[data-testid="category-${category.category.toLowerCase()}"]`,
        );
        await expectElementText(
          page,
          `[data-testid="category-amount-${i}"]`,
          `$${category.amount}`,
        );
        await expectElementText(
          page,
          `[data-testid="category-percentage-${i}"]`,
          `${category.percentage}%`,
        );
      }
    ...
    test("should show personalized insights and recommendations", async ({
      page,
    }) => {
      await dashboardPage.goto();
      ...
      for (let i = 0; i < insights.length; i++) {
        ...
        await expect(
          insightElement.locator('[data-testid="insight-title"]'),
        ).toContainText(insight.title);
        await expect(
          insightElement.locator('[data-testid="insight-description"]'),
        ).toContainText(insight.description);
  ...
  test.describe("Detailed Reports Page", () => {
    test("should load comprehensive reports with performance", async ({
      page,
    }) => {
      ...
      await expectElementVisible(
        page,
        '[data-testid="monthly-trends-section"]',
      );
      await expectElementVisible(
        page,
        '[data-testid="category-analysis-section"]',
      );
      await expectElementVisible(
        page,
        '[data-testid="merchant-analysis-section"]',
      );
      await expectElementVisible(
        page,
        '[data-testid="spending-patterns-section"]',
      );
    ...
    test("should filter reports by date range", async ({ page }) => {
      ...
      await reportsPage.selectDateRange(
      ...
      await expectElementText(
        page,
        '[data-testid="filtered-total-spending"]',
        "$1,234.56",
      );
      await expectElementText(
        page,
        '[data-testid="filtered-total-receipts"]',
        "15",
      );
      ...
      for (const preset of presets) {
        ...
        await expectElementVisible(
          page,
          `[data-testid="active-preset-${preset.name}"]`,
        );
    ...
    test("should compare different time periods", async ({ page }) => {
      ...
      await expectElementText(
        page,
        '[data-testid="spending-change"]',
        "+25.0%",
      );
      await expectElementText(
        page,
        '[data-testid="receipts-change"]',
        "+25.0%",
      );
      ...
      await expectElementVisible(
        page,
        '[data-testid="spending-increase-indicator"]',
      );
      await expectElementVisible(
        page,
        '[data-testid="receipts-increase-indicator"]',
      );
    });
    ...
    test("should analyze spending by merchant", async ({ page }) => {
      ...
      for (let i = 0; i < merchantAnalysis.topMerchants.length; i++) {
        ...
        await expect(
          merchantRow.locator('[data-testid="merchant-name"]'),
        ).toContainText(merchant.name);
        await expect(
          merchantRow.locator('[data-testid="merchant-total"]'),
        ).toContainText(`$${merchant.totalSpent}`);
        await expect(
          merchantRow.locator('[data-testid="merchant-count"]'),
        ).toContainText(merchant.receiptCount.toString());
        await expect(
          merchantRow.locator('[data-testid="merchant-average"]'),
        ).toContainText(`$${merchant.avgSpent}`);
      }
      ...
      await expectElementText(
        ...
        merchantAnalysis.topMerchants[0].name,
      );
    ...
    test("should show spending patterns and habits", async ({ page }) => {
      const spendingPatterns = {
        timePatterns: {
          hourly: {
            ...
            data: [
              5, 3, 1, 0, 0, 2, 8, 15, 25, 30, 35, 40, 45, 42, 38, 35, 30, 25,
              20, 15, 12, 10, 8, 6,
            ],
          },
          weekly: {
      ...
      await expect(insightElements).toHaveCount(
        spendingPatterns.insights.length,
      );

      for (let i = 0; i < spendingPatterns.insights.length; i++) {
        await expect(insightElements.nth(i)).toContainText(
          spendingPatterns.insights[i],
        );
      }
      ...
      await expectElementVisible(
        page,
        '[data-testid="category-comparison-chart"]',
      );
  ...
  test.describe("Export and Sharing", () => {
    ...
    test("should share reports via email", async ({ page }) => {
      ...
      await page.fill(
        ...
        "Here is the monthly expense report",
      );
      ...
      await page.fill(
        ...
        "boss@company.com, accountant@company.com",
      );
    ...
    test("should create scheduled reports", async ({ page }) => {
      ...
      await expectElementVisible(
        page,
        '[data-testid="schedule-paused-indicator"]',
      );
    });
  });
  ...
  test.describe("Advanced Analytics Features", () => {
    test("should provide budget tracking and alerts", async ({ page }) => {
      ...
      for (let i = 0; i < budgetData.budgets.length; i++) {
        ...
        await expect(
          budgetCard.locator('[data-testid="budget-category"]'),
        ).toContainText(budget.category);
        await expect(
          budgetCard.locator('[data-testid="budget-spent"]'),
        ).toContainText(`$${budget.spentAmount}`);
        await expect(
          budgetCard.locator('[data-testid="budget-total"]'),
        ).toContainText(`$${budget.budgetAmount}`);
        await expect(
          budgetCard.locator('[data-testid="budget-percentage"]'),
        ).toContainText(`${budget.percentage}%`);

        // Verify status indicators
        await expectElementVisible(
          page,
          `[data-testid="budget-status-${budget.status}"]`,
        );
      }
    ...
    test("should show tax-related reporting features", async ({ page }) => {
      const taxData = {
        ...
        categories: [
          ...
          { name: "Travel", amount: 199.0, deductible: true, percentage: 100 },
          { name: "Medical", amount: 100.0, deductible: true, percentage: 100 },
        ],
        documents: [
          {
            receiptId: "receipt-1",
      ...
      await expectElementText(
        page,
        '[data-testid="total-deductible"]',
        `$${taxData.summary.totalDeductible}`,
      );
      await expectElementText(
        page,
        '[data-testid="business-expenses"]',
        `$${taxData.summary.businessExpenses}`,
      );
      ...
      for (let i = 0; i < taxData.categories.length; i++) {
        ...
        await expect(
          categoryRow.locator('[data-testid="category-name"]'),
        ).toContainText(category.name);
        await expect(
          categoryRow.locator('[data-testid="category-amount"]'),
        ).toContainText(`$${category.amount}`);
        await expect(
          categoryRow.locator('[data-testid="deductible-percentage"]'),
        ).toContainText(`${category.percentage}%`);
      }
      ...
      await expectElementVisible(
        page,
        '[data-testid="tax-categorization-tool"]',
      );

      const uncategorizedReceipts = page.locator(
        '[data-testid="uncategorized-receipt"]',
      );
    ...
    test("should provide forecasting and predictions", async ({ page }) => {
      ...
      await expectElementText(
        page,
        '[data-testid="next-month-prediction"]',
        `$${forecastData.spending.nextMonth.predicted}`,
      );
      await expectElementText(
        page,
        '[data-testid="prediction-confidence"]',
        `${forecastData.spending.nextMonth.confidence}%`,
      );

      // Verify forecast range
      await expectElementText(
        page,
        '[data-testid="forecast-range-low"]',
        `$${forecastData.spending.nextMonth.range.low}`,
      );
      await expectElementText(
        page,
        '[data-testid="forecast-range-high"]',
        `$${forecastData.spending.nextMonth.range.high}`,
      );
      ...
      for (let i = 0; i < forecastData.trends.categories.length; i++) {
        ...
        await expect(
          trendElement.locator('[data-testid="trend-category"]'),
        ).toContainText(trend.name);
        await expect(
          trendElement.locator('[data-testid="trend-direction"]'),
        ).toContainText(trend.trend);
        await expect(
          trendElement.locator('[data-testid="trend-change"]'),
        ).toContainText(`${Math.abs(trend.change)}%`);
      }
      ...
      await expectElementVisible(
        page,
        '[data-testid="forecast-adjustment-dialog"]',
      );
    ...
    test("should handle large datasets efficiently", async ({ page }) => {
      ...
      const largeDataset = {
        receipts: Array.from({ length: 1000 }, (_, i) => ({
          ...
          category: ["Groceries", "Dining", "Shopping", "Transportation"][
            i % 4
          ],
        })),
        aggregatedData: {
      ...
      await expectElementVisible(page, '[data-testid="large-dataset-loaded"]', {
        timeout: 10000,
      });
      ...
      const scrollContainer = page.locator(
        '[data-testid="data-scroll-container"]',
      );
