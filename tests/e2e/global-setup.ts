...
async function globalSetup(config: FullConfig) {
  ...
  try {
    ...
    await page.waitForSelector('[data-testid="app-root"], #app, body', {
      timeout: 30000,
    });
  ...
  for (const user of testUsers) {
    try {
      ...
      await page.waitForSelector(
        '[data-testid="registration-success"], [data-testid="user-exists-error"]',
        { timeout: 10000 },
      );
  ...
  try {
    ...
    for (const receipt of sampleReceipts) {
      try {
        ...
        await page.waitForSelector('[data-testid="receipt-saved"]', {
          timeout: 5000,
        });
        ...
        console.log(
          `ℹ️ Could not create test receipt ${receipt.name}:`,
          error.message,
        );
      }
    }
