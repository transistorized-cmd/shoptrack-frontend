...
test.describe("Basic Vue Application Performance", () => {
    ...
    await page.addInitScript(() => {
      ...
      window.fetch = async function (...args) {
        ...
        try {
          ...
          const requestIndex =
            window.performanceMetrics.networkRequests.length - 1;
          window.performanceMetrics.networkRequests[requestIndex].endTime =
            endTime;
          window.performanceMetrics.networkRequests[requestIndex].duration =
            endTime - startTime;
          window.performanceMetrics.networkRequests[requestIndex].status =
            response.status;
          return response;
        } catch (error) {
          const endTime = performance.now();
          ...
          window.performanceMetrics.networkRequests[requestIndex].duration =
            endTime - startTime;
          window.performanceMetrics.networkRequests[requestIndex].error =
            error.message;
          throw error;
        }
      };
  ...
  test("should measure Vue app initial load performance", async ({ page }) => {
    ...
    if (performanceMetrics.memoryUsage) {
      console.log(
        `  Memory Usage: ${performanceMetrics.memoryUsage.used}MB / ${performanceMetrics.memoryUsage.total}MB`,
      );
  ...
  test("should measure bundle size and loading performance", async ({
    page,
  }) => {
    ...
    console.log(
      `  JS Bundle Size: ${(bundleAnalysis.totalJSSize / 1024).toFixed(1)}KB`,
    );
    console.log(
      `  CSS Bundle Size: ${(bundleAnalysis.totalCSSSize / 1024).toFixed(1)}KB`,
    );
    console.log(
      `  Total Bundle Size: ${(bundleAnalysis.totalBundleSize / 1024).toFixed(1)}KB`,
    );
  ...
  test("should measure Vue component mounting performance", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    if (vueMetrics.componentMounts.length > 0) {
      const totalMountTime = vueMetrics.componentMounts.reduce(
        (sum, mount) => sum + mount.mountTime,
        0,
      );
      const averageMountTime =
        totalMountTime / vueMetrics.componentMounts.length;
  ...
  test("should generate comprehensive performance report", async ({ page }) => {
    ...
    const performanceReport = await page.evaluate((testTime) => {
      ...
      const cssResources = resourceEntries.filter((r) =>
        r.name.includes(".css"),
      );
    ...
    console.log(`\n⏱️  Timing Metrics:`);
    console.log(
      `  First Paint: ${performanceReport.timing.firstPaint.toFixed(1)}ms`,
    );
    console.log(
      `  First Contentful Paint: ${performanceReport.timing.firstContentfulPaint.toFixed(1)}ms`,
    );
    console.log(
      `  DOM Content Loaded: ${performanceReport.timing.domContentLoaded.toFixed(1)}ms`,
    );
    console.log(
      `  Load Complete: ${performanceReport.timing.loadComplete.toFixed(1)}ms`,
    );

    console.log(`\n📦 Resource Analysis:`);
    console.log(
      `  Total Requests: ${performanceReport.resources.totalRequests}`,
    );
    console.log(
      `  JavaScript Files: ${performanceReport.resources.jsRequests}`,
    );
    console.log(`  CSS Files: ${performanceReport.resources.cssRequests}`);
    console.log(
      `  Total Transfer Size: ${(performanceReport.resources.totalTransferSize / 1024).toFixed(1)}KB`,
    );
