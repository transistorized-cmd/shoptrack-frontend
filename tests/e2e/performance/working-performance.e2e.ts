...
test.describe("Working Vue Application Performance Tests", () => {
  test("should analyze bundle loading and performance metrics", async ({
    page,
  }) => {
    ...
    const bundleAnalysis = {
      totalJSSize: resourceMetrics.jsFiles.reduce(
        (sum, file) => sum + file.size,
        0,
      ),
      totalCSSSize: resourceMetrics.cssFiles.reduce(
        (sum, file) => sum + file.size,
        0,
      ),
      totalImageSize: resourceMetrics.imageFiles.reduce(
        (sum, file) => sum + file.size,
        0,
      ),
      totalFontSize: resourceMetrics.fontFiles.reduce(
        (sum, file) => sum + file.size,
        0,
      ),
      ...
      averageLoadTime:
        resourceMetrics.loadTimes.length > 0
          ? resourceMetrics.loadTimes.reduce((sum, time) => sum + time, 0) /
            resourceMetrics.loadTimes.length
          : 0,
    };
    ...
    console.log(
      `  Document Ready State: ${performanceMetrics.documentReadyState}`,
    );
    console.log(
      `  Vue Detected: ${performanceMetrics.vueDetected ? "Yes" : "No"}`,
    );
    console.log(
      `  Has Content: ${performanceMetrics.hasContent ? "Yes" : "No"}`,
    );

    console.log(`\n⏱️  Timing Metrics:`);
    console.log(`  First Paint: ${performanceMetrics.firstPaint.toFixed(1)}ms`);
    console.log(
      `  First Contentful Paint: ${performanceMetrics.firstContentfulPaint.toFixed(1)}ms`,
    );
    console.log(
      `  DOM Content Loaded: ${performanceMetrics.domContentLoaded.toFixed(1)}ms`,
    );
    console.log(
      `  Load Complete: ${performanceMetrics.loadComplete.toFixed(1)}ms`,
    );

    console.log(`\n📦 Bundle Analysis:`);
    console.log(
      `  JavaScript: ${bundleAnalysis.jsFileCount} files, ${(bundleAnalysis.totalJSSize / 1024).toFixed(1)}KB`,
    );
    console.log(
      `  CSS: ${bundleAnalysis.cssFileCount} files, ${(bundleAnalysis.totalCSSSize / 1024).toFixed(1)}KB`,
    );
    console.log(
      `  Images: ${bundleAnalysis.imageFileCount} files, ${(bundleAnalysis.totalImageSize / 1024).toFixed(1)}KB`,
    );
    console.log(
      `  Fonts: ${bundleAnalysis.fontFileCount} files, ${(bundleAnalysis.totalFontSize / 1024).toFixed(1)}KB`,
    );
    console.log(
      `  Total Bundle: ${(resourceMetrics.totalSize / 1024).toFixed(1)}KB`,
    );
    console.log(
      `  Average Resource Load Time: ${bundleAnalysis.averageLoadTime.toFixed(1)}ms`,
    );
    ...
    console.log(
      `  Network Resources Tracked: ${resourceMetrics.jsFiles.length + resourceMetrics.cssFiles.length + resourceMetrics.imageFiles.length + resourceMetrics.fontFiles.length}`,
    );
    ...
    if (bundleAnalysis.totalJSSize > 1024 * 1024) {
      // 1MB
      performanceGrade = "B";
    ...
    allResources.forEach((resource, index) => {
      console.log(
        `    ${index + 1}. ${resource.url}: ${(resource.size / 1024).toFixed(1)}KB`,
      );
    });
    ...
    await page.evaluate(
      (data) => {
        const performanceBaseline = {
          ...
          bundleSize:
            data.bundleAnalysis.totalJSSize + data.bundleAnalysis.totalCSSSize,
  ...
  test("should measure progressive loading performance", async ({ page }) => {
    ...
    console.log(
      `  Time to First Contentful Paint: ${milestones.firstContentfulPaint.toFixed(1)}ms`,
    );
    console.log(
      `  DOM Content Loaded: ${milestones.domContentLoaded.toFixed(1)}ms`,
    );
    ...
    const phases = {
      ...
      domProcessing:
        milestones.domContentLoaded - milestones.firstContentfulPaint,
    ...
    if (phases.resourceLoading > 3000)
      bottlenecks.push("Slow resource loading");
