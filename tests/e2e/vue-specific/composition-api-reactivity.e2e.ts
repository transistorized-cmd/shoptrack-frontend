...
test.describe("Vue 3 Composition API Reactivity", () => {
  ...
  test.describe("Cross-Component Data Reactivity", () => {
    test("should sync reactive data between dashboard and receipts components", async ({
      page,
    }) => {
      ...
      await expectElementText(
        page,
        '[data-testid="total-receipts-count"]',
        "1",
      );
      await expectElementText(
        page,
      ...
      await expectElementText(
        page,
        '[data-testid="receipts-summary-count"]',
        "1",
      );
      await expectElementText(
        page,
        '[data-testid="receipts-summary-total"]',
        "$25.99",
      );
      ...
      await expectElementVisible(
        page,
        '[data-testid="receipt-item-receipt-2"]',
      );
      await expectElementText(
        page,
        '[data-testid="receipt-item-receipt-2"] [data-testid="merchant-name"]',
        "New Store",
      );
    });

    test("should handle reactive computed properties across components", async ({
      page,
    }) => {
      ...
      await expectElementText(
        page,
        '[data-testid="category-groceries-total"]',
        "$50.00",
      );
      await expectElementText(
        page,
        '[data-testid="category-fuel-total"]',
        "$30.00",
      );
      await expectElementText(
        page,
        '[data-testid="category-dining-total"]',
        "$25.00",
      );
      ...
      await expectElementText(
        page,
        '[data-testid="report-category-groceries"]',
        "$50.00",
      );
      await expectElementText(
        page,
        '[data-testid="report-category-fuel"]',
        "$30.00",
      );
      await expectElementText(
        page,
        '[data-testid="report-category-dining"]',
        "$25.00",
      );
      ...
      await page.goto("/receipts");
      await page.click(
        '[data-testid="receipt-item-receipt-1"] [data-testid="edit-receipt"]',
      );
      ...
      await mockApiResponse(page, "**/api/receipts", {
        ...
        total: 3,
        categoryTotals: {
          groceries: 0.0, // Now zero
          fuel: 30.0,
          dining: 75.0, // Now 50 + 25
        },
      });
    ...
    test("should maintain reactivity with deep reactive objects", async ({
      page,
    }) => {
      ...
      await mockApiResponse(page, "**/api/receipts/receipt-1", {
        ...
        items: [
          { id: "item-1", name: "Product A", price: 25.0, quantity: 2 },
      ...
      await mockApiResponse(page, "**/api/receipts/receipt-1", {
        id: "receipt-1",
        merchant: "Detailed Store",
        total: 125.0, // Updated total: (25*3) + 50
        metadata: {
          paymentMethod: "credit",
          cashier: "John Doe",
          location: {
        ...
        items: [
          { id: "item-1", name: "Product A", price: 25.0, quantity: 3 }, // Updated quantity
          { id: "item-2", name: "Product B", price: 50.0, quantity: 1 },
        ],
      });
      ...
      await expectElementText(
        page,
        '[data-testid="report-receipt-total"]',
        "$125.00",
      );
      await expectElementText(
        page,
        '[data-testid="report-store-name"]',
        "Store #456",
      );
  ...
  test.describe("Reactive Refs and Computed Dependencies", () => {
    test("should handle ref dependencies across components", async ({
      page,
    }) => {
      ...
      await expectElementVisible(
        page,
        '[data-testid="receipt-item-receipt-1"]',
      );
      await expectElementVisible(
        page,
        '[data-testid="receipt-item-receipt-3"]',
      );
      await expect(
        page.locator('[data-testid="receipt-item-receipt-2"]'),
      ).not.toBeVisible();
      ...
      await expectElementText(
        page,
        '[data-testid="filtered-receipts-count"]',
        "2",
      );
      await expectElementText(
        page,
      ...
      await expect(
        page.locator('[data-testid="receipt-item-receipt-1"]'),
      ).not.toBeVisible();
    });

    test("should handle computed property chains across components", async ({
      page,
    }) => {
        ...
        budgets: [
          {
            category: "groceries",
            monthlyLimit: 250.0,
            currentSpending: 300.0,
          },
          { category: "dining", monthlyLimit: 200.0, currentSpending: 150.0 },
        ],
      });
      ...
      await expectElementText(
        page,
        '[data-testid="groceries-status"]',
        "Over Budget",
      );
      await expectElementText(
        page,
        '[data-testid="groceries-overage"]',
        "$50.00",
      ); // 300 - 250
      await expectElementText(
        page,
        '[data-testid="dining-status"]',
        "On Track",
      );
      await expectElementText(
        page,
        '[data-testid="dining-remaining"]',
        "$50.00",
      ); // 200 - 150
      ...
      await expectElementVisible(
        page,
        '[data-testid="budget-alert-groceries"]',
      );
      await expectElementText(
        page,
        '[data-testid="budget-alert-message"]',
        "You are $50.00 over budget in Groceries",
      );
      ...
      await mockApiResponse(page, "**/api/budgets", {
        budgets: [
          ...
          { category: "dining", monthlyLimit: 200.0, currentSpending: 225.0 }, // 150 + 75
        ],
      });
      ...
      const alertCount = await page
        .locator('[data-testid^="budget-alert-"]')
        .count();
  ...
  test.describe("Reactive State Management (Pinia Integration)", () => {
    test("should maintain reactive state across store mutations", async ({
      page,
    }) => {
  ...
  test.describe("Watch and WatchEffect Reactivity", () => {
    test("should handle watchers across component lifecycle", async ({
      page,
    }) => {
      ...
      await mockApiResponse(page, "**/api/analytics/range**", {
        ...
        averagePerDay: 16.13,
        trends: [
          { date: "2024-01-01", spending: 50.0 },
          { date: "2024-01-15", spending: 75.0 },
          { date: "2024-01-31", spending: 60.0 },
        ],
      });
      ...
      await expectElementText(
        page,
        '[data-testid="trends-period-total"]',
        "$500.00",
      );
    ...
    test("should handle watchEffect cleanup across route changes", async ({
      page,
    }) => {
      ...
      await page.type('[data-testid="live-search-input"]', "wal", {
        delay: 100,
      });
      ...
      // Continue typing
      await page.type('[data-testid="live-search-input"]', "mart", {
        delay: 100,
      });
  ...
  test.describe("Error Handling in Reactive Systems", () => {
    test("should handle reactive errors gracefully across components", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="receipts-error"]'),
      ).not.toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="dashboard-error"]'),
      ).not.toBeVisible();
      await expectElementText(
        page,
