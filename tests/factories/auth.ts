...
import { createFactory, testId, testDate, testEmail, testConstants } from './base';
import type {
  User,
  ConnectedAccount,
  AuthResponse,
 ...
 */
export const changePasswordRequestFactory = createFactory<ChangePasswordRequest>(() => ({
 ...
 */
export const forgotPasswordRequestFactory = createFactory<ForgotPasswordRequest>(() => ({
  email: testEmail.fake(),
}));
...
export const userVariants = {
   ...
   */
  unconfirmedEmail: () => userFactory.build({
   ...
   */
  withPasskeys: () => userFactory.build({
...
export const authResponseVariants = {
   ...
   */
  success: () => authResponseFactory.build({
    success: true,
   ...
   */
  failure: () => authResponseFactory.build({
    success: false,
   ...
   */
  emailConfirmationRequired: () => authResponseFactory.build({
    success: false,
   ...
   */
  twoFactorRequired: () => authResponseFactory.build({
    success: false,
   ...
   * Account suspended
   */
  suspended: () => authResponseFactory.build({
    success: false,
...
export const mockAuthServiceResponses = {
  ...
  error: {
    ...
    changePassword: () => Promise.reject(new Error('Current password incorrect')),
    logout: () => Promise.reject(new Error('Logout failed')),
    initializeAuth: () => Promise.reject(new Error('Auth initialization failed')),
    ...
    resendEmailConfirmation: () => Promise.reject(new Error('Too many requests')),
    verifyTwoFactor: () => Promise.reject(new Error('Invalid verification code')),
  },
};
