...
export const pluginFactory = createFactory<Plugin>(() => ({
  ...
  type: faker.helpers.arrayElement(['receipt-processor', 'data-enhancer', 'export-tool']),
  status: faker.helpers.arrayElement(['active', 'inactive', 'error', 'updating']),
  version: faker.system.semver(),
...
export const asyncJobFactory = createFactory<AsyncJob>(() => ({
  id: testId.uuid(),
  type: faker.helpers.arrayElement(['file-upload', 'receipt-process', 'data-export', 'plugin-install']),
...
export const notificationFactory = createFactory<Notification>(() => ({
  ...
  metadata: {
    ...
    category: faker.helpers.arrayElement(['upload', 'processing', 'security', 'account']),
  },
}));
...
export const pluginVariants = {
   ...
   */
  builtInProcessor: () => pluginFactory.build({
   ...
   * Third-party plugin
   */
  thirdParty: () => pluginFactory.build({
    isBuiltIn: false,
    status: 'active',
...
export const jobVariants = {
   ...
   * File upload job
   */
  fileUpload: () => asyncJobFactory.build({
    type: 'file-upload',
    status: 'running',
   ...
   * Completed upload
   */
  uploadCompleted: () => asyncJobFactory.build({
    type: 'file-upload',
    status: 'completed',
    progress: 100,
   ...
   * Failed upload
   */
  uploadFailed: () => asyncJobFactory.build({
    type: 'file-upload',
    status: 'failed',
    progress: 0,
   ...
   */
  receiptProcessing: () => asyncJobFactory.build({
   ...
   */
  processingCompleted: () => asyncJobFactory.build({
    type: 'receipt-process',
    status: 'completed',
    progress: 100,
   ...
   * Long-running job
   */
  longRunning: () => asyncJobFactory.build({
    status: 'running',
...
export const notificationVariants = {
   ...
   */
  success: () => notificationFactory.build({
    type: 'success',
   ...
   * Error notification
   */
  error: () => notificationFactory.build({
    type: 'error',
   ...
   */
  warning: () => notificationFactory.build({
    type: 'warning',
    title: 'Low Confidence Results',
    message: 'Some items were detected with low confidence. Please review and correct.',
    isRead: false,
   ...
   * System notification
   */
  system: () => notificationFactory.build({
    type: 'info',
   ...
   */
  expired: () => notificationFactory.build({
...
export const mockPluginServiceResponses = {
  success: {
    ...
    uploadWithAutoDetection: () => Promise.resolve(jobVariants.uploadCompleted()),
  },
  error: {
