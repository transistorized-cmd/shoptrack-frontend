...
import { createFactory, testId, testDate, testMoney, testConstants } from './base';
import type {
  Receipt,
  ReceiptItem,
  ReceiptQuery,
...
export const claudeResponseFactory = createFactory<ClaudeResponse>(() => ({
  ...
  imageQuality: faker.helpers.arrayElement(['poor', 'fair', 'good', 'excellent']),
}));
...
export const receiptItemFactory = createFactory<ReceiptItem>(() => ({
  ...
  weightNormalizedKg: faker.number.float({ min: 0.1, max: 5, fractionDigits: 2 }),
...
export const receiptFactory = createFactory<Receipt>(() => {
  const items = receiptItemFactory.buildList(faker.number.int({ min: 1, max: 8 }));
  const itemsDetected = items.length;
  const successfullyParsed = faker.number.int({ min: Math.floor(itemsDetected * 0.7), max: itemsDetected });
  ...
  return {
    ...
    imageQualityAssessment: faker.helpers.arrayElement(['poor', 'fair', 'good', 'excellent']),
...
export const receiptVariants = {
   ...
   */
  completed: () => receiptFactory.build({
    id: 1,
   ...
   */
  processing: () => receiptFactory.build({
   ...
   */
  partialSuccess: () => receiptFactory.build({
    processingStatus: testConstants.PROCESSING_STATUS.COMPLETED,
   ...
   */
  highConfidence: () => receiptFactory.build({
    processingStatus: testConstants.PROCESSING_STATUS.COMPLETED,
    items: receiptItemFactory.buildList(3, { confidence: testConstants.CONFIDENCE_LEVELS.HIGH }),
    claudeResponseJson: claudeResponseFactory.build({ confidence: testConstants.CONFIDENCE_LEVELS.HIGH }),
  }),

  /**
   * Receipt with low-confidence items
   */
  lowConfidence: () => receiptFactory.build({
    processingStatus: testConstants.PROCESSING_STATUS.COMPLETED,
    items: receiptItemFactory.buildList(3, { confidence: testConstants.CONFIDENCE_LEVELS.LOW }),
    claudeResponseJson: claudeResponseFactory.build({ confidence: testConstants.CONFIDENCE_LEVELS.LOW }),
  }),

  /**
   * Large receipt with many items
   */
  large: () => receiptFactory.build({
    processingStatus: testConstants.PROCESSING_STATUS.COMPLETED,
...
export const receiptItemVariants = {
   ...
   * Simple grocery item
   */
  groceryItem: () => receiptItemFactory.build({
    id: 1,
    receiptId: 1,
   ...
   * Electronics item
   */
  electronicsItem: () => receiptItemFactory.build({
   ...
   */
  weightedItem: () => receiptItemFactory.build({
   ...
   */
  manualItem: () => receiptItemFactory.build({
   ...
   */
  needsReview: () => receiptItemFactory.build({
...
export const mockReceiptsServiceResponses = {
  ...
  success: {
    ...
    getReceiptStats: () => Promise.resolve({
      totalReceipts: 50,
      totalItems: 250,
      totalAmount: 1250.99,
      averageAmount: 25.02,
