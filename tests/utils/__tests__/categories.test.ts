...
describe("Test Categorization System", () => {
  ...
  describe("shouldRunTest", () => {
    ...
    it("should exclude tests matching EXCLUDE_CATEGORIES", () => {
      ...
      expect(shouldRunTest([TestCategory.UNIT, TestCategory.FLAKY])).toBe(
        false,
      );
    });
    ...
    it("should handle multiple categories correctly", () => {
      ...
      expect(shouldRunTest([TestCategory.UNIT, TestCategory.COMPONENT])).toBe(
        true,
      );
      expect(shouldRunTest([TestCategory.UNIT, TestCategory.VIEW])).toBe(true); // Has unit
      expect(shouldRunTest([TestCategory.VIEW, TestCategory.INTEGRATION])).toBe(
        false,
      ); // Has neither
    });
  });
  ...
  describe("configureTestSuite", () => {
    it("should use shouldRunTest logic for suite filtering", () => {
      ...
      expect(shouldRunTest([TestCategory.PERFORMANCE, TestCategory.SLOW])).toBe(
        true,
      );
  ...
  describe("Integration with real test scenarios", () => {
    it("should support typical development workflow", () => {
      ...
      expect(shouldRunTest(CategoryCombos.UNIT_COMPONENT_FAST)).toBe(true);
      expect(shouldRunTest([TestCategory.INTEGRATION, TestCategory.SLOW])).toBe(
        false,
      );
      expect(shouldRunTest([TestCategory.UNIT, TestCategory.FLAKY])).toBe(
        false,
    ...
    it("should support CI pipeline configuration", () => {
      ...
      expect(shouldRunTest([TestCategory.UNIT, TestCategory.STABLE])).toBe(
        true,
      );
      expect(
        shouldRunTest([TestCategory.INTEGRATION, TestCategory.STABLE]),
      ).toBe(true);
      expect(
        shouldRunTest([TestCategory.UNIT, TestCategory.EXPERIMENTAL]),
      ).toBe(false);
    });
